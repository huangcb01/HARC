...
while [[ $# -gt 0 ]]; do
  case $1 in
    --max_samples_per_domain)      MAX_SAMPLES_PER_DOMAIN="$2"; shift 2 ;;
...
OUTPUT_PATH=$BASE_MODEL_PATH/${METHOD}-${REGULARIZATION}-${W_INIT}-${PRECONDITIONER}-${MAX_ITER}-${CG_TOL}-UltraFeedback_OpenMathInstruct2_SelfOSSInstructSC2_correct/${MAX_SAMPLES_PER_DOMAIN}
