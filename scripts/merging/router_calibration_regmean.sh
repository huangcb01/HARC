...
METHOD=router_calibration_regmean
PRECONDITIONER=none
REGULARIZATION=0.9
W_INIT=origin
MAX_ITER=1000
...
BASE_MODEL_PATH=output/OLMoE-1B-7B-0125-mergekit/math-code/linear-bfloat16
SOURCE_MODELS="[
    '$MODEL_DIR/math/full_bs-32_lr-2e-5-linear_epochs-2_liger_z2',
    '$MODEL_DIR/code/full_bs-32_lr-2e-5-linear_epochs-2_liger_z2',
]"
OUTPUT_PATH=$BASE_MODEL_PATH/${METHOD}-${REGULARIZATION}-${W_INIT}-${PRECONDITIONER}-${MAX_ITER}-${CG_TOL}-OpenMathInstruct2_SelfOSSInstructSC2_correct
