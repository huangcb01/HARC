...
TEMPLATE=olmo
DATA_DIR=$HDFS_DATA/moe-merge/sft
DATASETS=(math)
...
OUTPUT_DIR=$HDFS_OUTPUT/moe-merge-new/${MODEL_NAME}/$(IFS=-; echo "${DATASETS[*]}")/full_bs-${TOTAL_BATCH_SIZE}_lr-${LEARNING_RATE}-${LR_SCHEDULER_TYPE}_epochs-${NUM_TRAIN_EPOCHS}_liger_z2_packing_balance
